...
private forceGroundNamedDecorations(namePartials: string[], groundMeshes: THREE.Mesh[]): void {
  if (!this.worldRoot || !namePartials.length || !groundMeshes.length) return;

  const ray = new THREE.Raycaster();
  const DOWN = new THREE.Vector3(0, -1, 0);

  // Reuse the same grouping heuristic as dropFloatingDecorations so multi-mesh props stay intact.
  const pickMoveNode = (mesh: THREE.Object3D): THREE.Object3D => {
    const p = mesh.parent as THREE.Object3D | null;
    if (!p) return mesh;
    if (this.worldRoot && p === this.worldRoot) return mesh;
    if ((p as any).children && (p as any).children.length > 1 && (p as any).children.length <= 12) return p;
    return mesh;
  };

  const findBest = (partial: string): THREE.Object3D | null => {
    let best: THREE.Object3D | null = null;
    let bestVol = 0;
    this.worldRoot!.traverse((o) => {
      const n = (o as any).name as string | undefined;
      if (!n) return;
      if (!n.includes(partial)) return;
      // Prefer the largest object by AABB volume.
  ...
  for (const partial of namePartials) {
    const found = findBest(partial);
    if (!found) continue;

    const node = pickMoveNode(found);
    const box = new THREE.Box3().setFromObject(node);
    const size = new THREE.Vector3();
    const center = new THREE.Vector3();
    box.getSize(size);
    ...
    // Build a raycast candidate list that excludes the decoration itself.
    const exclude = new Set<string>();
    node.traverse((o) => exclude.add(o.uuid));
    const candidates = groundMeshes.filter((m) => !exclude.has(m.uuid));
    ...
    // Allow larger corrections here; these two props have been observed far above ground.
    const maxGap = 80.0;
    if (gap > maxGap) continue;
...
const mk = (sign: number) => {
  ...
  private addBoxCollider(mesh: THREE.Mesh, opts: { friction: number; shrink: number }): void {
    ...
    // SWEETLAND_LADDER_NO_COLLIDER_V1
    // Ladders already have climb volumes (this.ladders). Giving the ladder mesh a solid box collider
    // makes the player capsule snag/bounce on the rounded top while still in climb mode.
    // Keep the ladder visible, but remove its solid collider.
    if (/(ladder|climb|rope|rung)/i.test(fullName)) return;
...
private addJumpBridgeSlab(root: THREE.Object3D): void {
  // Adds one or more "bridge" slabs that the player can jump to.
  // Uses a saved anchor point (localStorage) and tries to find a nearby waffle slab (or any waffle mesh) as the target.
  // Then spawns TWO mid slabs (1/3 and 2/3) between the anchor and the target.
  //
  // SWEETLAND_DOUBLE_BRIDGE_SLABS_V1

  const getLS = (k: string) => {
    try { return window.localStorage.getItem(k); } catch { return null; }
  };

  const parseVec3 = (raw: string | null): THREE.Vector3 | null => {
    if (!raw) return null;
    try {
      ...
      if (parts.length === 3 && parts.every((n) => Number.isFinite(n))) return new THREE.Vector3(parts[0], parts[1], parts[2]);
      return null;
    }
  };

  // ---- Anchor (saved by earlier tooling) ----
  const anchor =
    parseVec3(getLS("sweetland:slab02")) ??
    parseVec3(getLS("sweetland:slab02Override")) ??
    parseVec3(getLS("sweetland:slab02_override")) ??
    parseVec3(getLS("sweetland_slab02")) ??
    parseVec3(getLS("sweetland_slab02_override")) ??
    parseVec3(getLS("sweetland_slab02Override")) ??
    parseVec3(getLS("sweetland_slab02Pos")) ??
    parseVec3(getLS("sweetland:slab02Pos"));

  if (!anchor) return;

  // Helper: find a "template mesh" from an arbitrary Object3D
  const firstMesh = (obj: THREE.Object3D | null | undefined): THREE.Mesh | null => {
    if (!obj) return null;
    if ((obj as any).isMesh) return obj as any as THREE.Mesh;
    let found: THREE.Mesh | null = null;
    obj.traverse((o: any) => {
      if (found) return;
      if (o && o.isMesh && o.geometry) found = o as THREE.Mesh;
    });
    return found;
  };

  // Helper: compute pick-info needed downstream
  const pickFromObject = (obj: THREE.Object3D): any => {
    ...
    // ---- Choose the correct waffle "solid" ----
  // There can be multiple waffle slabs in the scene. Previously we picked the closest-to-anchor in full 3D,
  // which can accidentally lock onto the wrong waffle (e.g. near the candy-cane arch).
  //
  // New behavior:
  //  - If localStorage has sweetland:waffleTarget (vec3), pick the waffle closest to THAT point in XZ.
  //  - Otherwise pick the waffle closest to the anchor in XZ.
  const waffleTarget =
    parseVec3(getLS("sweetland:waffleTarget")) ??
    parseVec3(getLS("sweetland_waffleTarget")) ??
    parseVec3(getLS("sweetland:waffle_target")) ??
    parseVec3(getLS("sweetland_waffle_target"));

  const targetRef = (waffleTarget ?? anchor).clone();

  const xzD2 = (p: THREE.Vector3, q: THREE.Vector3) => {
    const dx = p.x - q.x;
    const dz = p.z - q.z;
    return dx * dx + dz * dz;
  };

  let best: { obj: THREE.Object3D; d2: number; center: THREE.Vector3 } | null = null;

  this.scene.traverse((o: any) => {
    const name = o?.name ? String(o.name).toLowerCase() : "";
    if (!name) return;
    if (name.indexOf("waffle") === -1) return;
    ...
    const d2 = xzD2(c, targetRef);
    if (!best || d2 < best.d2) best = { obj: o, d2, center: c.clone() };
  });

  if (!best) {
    console.warn("[SweetLand] Bridge: could not find any waffle slab candidates.");
    return;
  }

  if (waffleTarget) {
    console.log("[SweetLand] Bridge: using waffleTarget", waffleTarget, "picked waffle", best.obj?.name || "(unnamed)", "center", best.center);
  } else {
    console.log("[SweetLand] Bridge: picked waffle near anchor", best.obj?.name || "(unnamed)", "center", best.center);
  }

  const slabPick: any = pickFromObject(best.obj);
  if (!slabPick) {
    console.warn("[SweetLand] Waffle candidate existed but had no mesh/geometry; skipping slab02.");
    return;
  }

  // Defensive: ensure required fields exist
  if (!slabPick.center || !slabPick.mesh || !slabPick.mesh.geometry) {
    console.warn("[SweetLand] Invalid slabPick for slab02; skipping.", slabPick);
    return;
  }

  // Place slab02 between anchor and slabPick, at the slabPick height.
  const slabTopY = Number.isFinite(slabPick.topY) ? slabPick.topY : anchor.y;
  const slabCenterY = slabTopY - (Number.isFinite(slabPick.sy) ? slabPick.sy / 2 : 0);

  const a = anchor.clone();
  a.y = slabTopY;

  const s = (slabPick.center.clone ? slabPick.center.clone() : new THREE.Vector3(slabPick.center.x, slabPick.center.y, slabPick.center.z));
  s.y = slabTopY;

  // Two intermediate slabs at 1/3 and 2/3
  const tList = [1 / 3, 2 / 3];
  const mids = tList.map((t) => {
    const m = a.clone().lerp(s, t);
    m.y = slabCenterY;
    return m;
  });

  const placeSlab = (name: string, at: THREE.Vector3) => {
    const templateMesh = slabPick.mesh as THREE.Mesh;

    // Visible mesh
    const slab = new THREE.Mesh(templateMesh.geometry, templateMesh.material as any);
    slab.name = name;
    slab.position.copy(at);

    // Apply world rotation/scale from the template
    if (slabPick.quat && slabPick.quat.isQuaternion) slab.quaternion.copy(slabPick.quat);
    if (slabPick.scale && slabPick.scale.isVector3) slab.scale.copy(slabPick.scale);

    (slab as any).castShadow = true;
    (slab as any).receiveShadow = true;
    this.scene.add(slab);

    // Static collider
    try {
      const sx = Number.isFinite(slabPick.sx) ? slabPick.sx : 1;
      const sy = Number.isFinite(slabPick.sy) ? slabPick.sy : 0.5;
      const sz = Number.isFinite(slabPick.sz) ? slabPick.sz : 1;
      const desc = this.physics.R.ColliderDesc.cuboid(sx / 2, sy / 2, sz / 2)
        .setTranslation(at.x, at.y, at.z)
        .setFriction(1.0);
      this.physics.world.createCollider(desc, this.staticBody);
    } catch (e) {
      console.warn("[SweetLand] Failed to create bridge slab collider:", e);
    }
  };

  placeSlab("SweetLand_BridgeSlab02", mids[0]);
  placeSlab("SweetLand_BridgeSlab03", mids[1]);

  console.log("[SweetLand] Added bridge slabs between anchor and waffle slab.", {
    anchor: { x: a.x, y: a.y, z: a.z },
    waffle: { x: s.x, y: s.y, z: s.z, topY: slabTopY },
    slab02: { x: mids[0].x, y: mids[0].y, z: mids[0].z },
    slab03: { x: mids[1].x, y: mids[1].y, z: mids[1].z }
  });
}
...
for (const s of npcSpots) {
  ...
  private spawnBridgeSlabClone(
    ...
    if (existing) {
    // SWEETLAND_JELLO_BRIDGE_CLONE_GUARD_V1
    // Guard against missing templates so we never call .clone() on undefined.
    if (!template || typeof (template as any).clone !== "function") {
      console.warn("[SweetLand] Jello bridge: missing/invalid template for cloning (expected .clone()). Skipping slab spawn.", {
        name,
        templateType: template ? (template as any).type : null,
        templateName: template ? (template as any).name : null,
      });
      return;
    }
